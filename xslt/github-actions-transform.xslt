...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="gh:workflow">
    <!-- Workflow name -->
    <xsl:if test="gh:name">
      <xsl:text>name: </xsl:text>
      <xsl:value-of select="gh:name"/>
      <xsl:text>&#10;&#10;</xsl:text>
    </xsl:if>
    ...
    <xsl:if test="gh:run-name">
      ...
      <xsl:value-of select="gh:run-name"/>
      <xsl:text>&#10;&#10;</xsl:text>
    </xsl:if>
  ...
  <xsl:template name="quote-if-needed">
    <xsl:param name="text"/>
    <xsl:choose>
      <xsl:when test="contains($text, ' ') or contains($text, ':') or contains($text, '#') or contains($text, &quot;'&quot;) or contains($text, '&quot;') or starts-with($text, '-') or starts-with($text, '[') or starts-with($text, '{') or starts-with($text, '$')">
        <xsl:text>"</xsl:text>
